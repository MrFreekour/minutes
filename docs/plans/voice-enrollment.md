...
3. **On-device, private by construction** — the voiceprint never leaves the machine (`~/.minutes/voices.db`, 0600, separate from the process-private relationship projection). Cloud tools can't say that.
...
- **Attribution already consumes profiles:** L2 `match_speakers_by_voice` (High/Enrollment) and self-anchoring `single_stem_speaker_self_attribution` consume trusted profiles. Legacy `confirm --save-voice` is now fail-closed because its ambient embedding sidecar has no exact source/model/sensitivity provenance; this plan must replace that bridge with provenance-bound samples before meeting-derived enrollment returns. Only **High**-confidence rewrites transcript labels (the "wrong names are worse than anonymous" rule stays intact).
...
- **Passive learning is CANDIDATE CAPTURE, not auto-enroll (P0, supersedes §4.5/§8).** Passively-captured embeddings are quarantined candidates that **never affect matching** until an explicit, multi-condition **promotion**. Passive capture defaults OFF unless the user has explicitly enabled biometric-template retention. Initially, manual enrollment creates trusted samples; meeting-derived confirmation remains disabled until it carries exact source, model, dimension, normalization, and sensitivity provenance. This kills the poisoning class at the root rather than mitigating it after the fact.
