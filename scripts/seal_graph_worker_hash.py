#!/usr/bin/env python3
"""Bind one exact signed graph helper to the parent Mach-O before outer signing."""
...
def seal(executable: pathlib.Path, cdhash: str, *, verify: bool) -> None:
    if re.fullmatch(r"[0-9a-f]{40}", cdhash) is None:
        raise SystemExit("graph worker CodeDirectory hash must be 40 lowercase hex bytes")

    expected = MARKER + cdhash.encode("ascii")
    contents = executable.read_bytes()
    if verify:
        if contents.count(expected) != 1 or PLACEHOLDER in contents:
            raise SystemExit("parent executable does not contain one exact graph-worker seal")
        return

    if contents.count(PLACEHOLDER) != 1:
        raise SystemExit("parent executable must contain one unbound graph-worker seal")
    if contents.count(MARKER) != 1:
        raise SystemExit("parent executable contains an ambiguous graph-worker seal")
    ...
    if sealed.count(expected) != 1 or PLACEHOLDER in sealed:
        raise SystemExit("parent executable graph-worker seal verification failed")
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        try:
            ...
            raise SystemExit("duplicate graph-worker seals were accepted")
        ...
        try:
            ...
            raise SystemExit("uppercase graph-worker hash was accepted")
