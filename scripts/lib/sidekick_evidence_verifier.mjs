...
const HUMAN_FALLBACK = [
  ...
  /\b(?:automat\w*|ship|launch|deploy)\b[^.!?\n]{0,45}\bonly\b[^.!?\n]{0,30}\b(?:confident|high[- ]confidence|above[- ]threshold)\b[^.!?\n]{0,55}\b(?:leave|keep|route|send|return)\b[^.!?\n]{0,35}\b(?:everything else|all others|all other (?:work|tickets?|cases?|items?|requests?|resolutions?)|the rest|the remainder|the balance)\b[^.!?\n]{0,30}\b(?:humans?|people|support (?:team|reps?|agents?)|specialists?|operators?)\b/i,
  /\b(?:rout(?:e|ing)|send|return|leave|keep|retain)\w*\b[^.!?\n]{0,35}\b(?:all others|all other (?:work|tickets?|cases?|items?|requests?|resolutions?))\b[^.!?\n]{0,30}\b(?:humans?|human (?:handling|review)|people|support (?:team|reps?|agents?)|specialists?|operators?)\b/i,
];
